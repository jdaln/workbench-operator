...
.PHONY: test
test: manifests generate fmt vet envtest ## Run tests, filtering out the e2e ones.
...
.PHONY: run-local
run-local: manifests generate fmt vet ## Run a controller from your host with local storage.
	@echo "Running with local storage for development..."
	go run ./cmd/main.go \
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name workbench-operator-builder
	$(CONTAINER_TOOL) buildx use workbench-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm workbench-operator-builder
	rm Dockerfile.cross
...
.PHONY: installdry
installdry: manifests kustomize ## create the CRD and only print the output
	$(KUSTOMIZE) build config/crd > $(OUT)
